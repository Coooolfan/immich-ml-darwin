...
class TestOrtSession:
    ...
    @pytest.mark.providers(COREML_EP)
    def test_uses_specific_provider_profile(self, providers: list[str], mocker: MockerFixture) -> None:
        mocker.patch.object(settings, "providers", ProviderProfiles(clip_visual="CPUExecutionProvider"))

        session = OrtSession("ViT-B-32__openai", provider_profile=["clip_visual", "clip"])

        assert session.providers == self.CPU_EP

    @pytest.mark.providers(COREML_EP)
    def test_uses_broader_provider_profile_if_specific_profile_is_not_set(
        self, providers: list[str], mocker: MockerFixture
    ) -> None:
        mocker.patch.object(settings, "providers", ProviderProfiles(clip="CPUExecutionProvider"))

        session = OrtSession("ViT-B-32__openai", provider_profile=["clip_visual", "clip"])

        assert session.providers == self.CPU_EP

    def test_retries_with_cpu_if_coreml_session_creation_fails(
        self, ort_session: mock.Mock, tmp_path: Path, mocker: MockerFixture
    ) -> None:
        model_path = tmp_path / "model.onnx"
        coreml_dir = tmp_path / "coreml"
        coreml_dir.mkdir()
        ort_session.side_effect = [ort.capi.onnxruntime_pybind11_state.Fail("bad coreml cache"), mock.Mock()]
        rmtree = mocker.patch("immich_ml.sessions.ort.rmtree")

        session = OrtSession(model_path, providers=self.COREML_EP)

        assert session.providers == self.CPU_EP
        assert ort_session.call_args_list[1].kwargs["providers"] == self.CPU_EP
        rmtree.assert_called_once_with(coreml_dir)
