...
class InferenceModel(ABC):
    ...
    def load(self) -> None:
        ...
        self.loaded = True
        log.info(
            f"Loaded {self.model_task}/{self.model_type} model '{self.model_name}' "
    ...
    def _make_session(self, model_path: Path) -> ModelSession:
        ...
        match model_path.suffix:
            ...
            case ".onnx":
                session = OrtSession(model_path, provider_profile=self.provider_profiles)
            case ".rknn":
    ...
    @property
    def provider_profiles(self) -> list[str]:
        task = self.model_task.value.replace("-", "_")
        model_type = self.model_type.value.replace("-", "_")
        return [f"{task}_{model_type}", task]
