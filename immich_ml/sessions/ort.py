...
class OrtSession:
    ...
    def __init__(
        ...
        provider_profile: str | list[str] | None = None,
    ):
        self.model_path = Path(model_path)
        if provider_profile is None:
            self.provider_profiles = []
        elif isinstance(provider_profile, str):
            self.provider_profiles = [provider_profile]
        else:
            self.provider_profiles = provider_profile
        ...
        self.session = self._create_session(self.providers, self.provider_options)
        log.info(f"Loaded ONNX session for {self.model_path} with active providers {self.session.get_providers()}")

    def _create_session(self, providers: list[str], provider_options: list[dict[str, Any]]) -> ort.InferenceSession:
        try:
            ...
            if not settings.coreml_retry_cpu_on_failure or "CoreMLExecutionProvider" not in providers:
                raise
            coreml_dir = self.model_path.parent / "coreml"
            log.warning(
                f"Failed to initialize CoreML session for {self.model_path}. "
                "Clearing CoreML cache and retrying with CPUExecutionProvider.",
                exc_info=True,
            )
            if coreml_dir.exists():
                rmtree(coreml_dir)
            self.providers = ["CPUExecutionProvider"]
            self.provider_options = self._provider_options_default
    ...
    def _providers_default(self) -> list[str]:
        ...
        if configured is not None:
            providers = [provider for provider in configured if provider in available_providers]
            skipped = [provider for provider in configured if provider not in available_providers]
            if skipped:
                log.warning(f"Ignoring unavailable ORT providers for {self.model_path}: {skipped}")
            if providers:
                log.info(f"Using provider profiles {self.provider_profiles} for {self.model_path}: {providers}")
                return providers
            log.warning(
                f"Provider profiles {self.provider_profiles} did not match any available ORT providers. "
                "Falling back to automatic provider selection."
            )
        return [provider for provider in SUPPORTED_PROVIDERS if provider in available_providers]

    @property
    def _configured_providers(self) -> list[str] | None:
        profile = None
        for provider_profile in self.provider_profiles:
            profile = getattr(settings.providers, provider_profile, None)
            if profile is not None:
                break
        if profile is None:
            profile = settings.providers.default
        if profile is None:
            return None

        return [provider.strip() for provider in profile.split(",") if provider.strip()]
